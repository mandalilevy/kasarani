...
if (isset($_POST['search'])) {
 ...
 $query ="SELECT * FROM `register` WHERE CONCAT(`name`,`id`,`date`,`gender`,`reason`,`education`,`school`,`place`,`cont`) LIKE '%".$valuetosearch."%'";
 $search_result=filtertable($query);
}
else{
$query="SELECT * FROM `register` ORDER BY `date`";
$search_result=filtertable($query);
}
...
<div class="container">
  <form action="registered_list.php" method="post">
<div class="form-row">
...
<?php 
$con=mysqli_connect("127.0.0.1","root","","kasarani");
$dddd="SELECT COUNT(id) AS total FROM `register` ";
